...
class _DeanHomePageState extends State<DeanHomePage> {
  ...
  // ===========================================================================
  // TAB 1: PERSONNEL (ADMIN LIFECYCLE)
  // ===========================================================================
...
// ---------------------------------------------------------------------------
// ADMIN LIST COMPONENT (Extracted for better state management)
// ---------------------------------------------------------------------------
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheetState) => Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("Onboard New Admin", style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Text("Create a standard admin account for a department.", style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: saving ? null : () async {
                    ...
                    if (!mounted) return;
                    Navigator.pop(ctx);
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Admin ${usernameCtrl.text.trim()} onboarded successfully.')));
                  },
                  ...
                  child: saving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: kDeanGold, strokeWidth: 2)) : const Text("Create Admin Profile", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                ),
              ),
              const SizedBox(height: 30),
            ],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text("Onboard Admin", style: TextStyle(fontWeight: FontWeight.bold)),
      ),
          ...
          if (admins.isEmpty) {
            return Center(child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const Text("No Administrators Found", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.grey)),
              ],
            ));
          }
